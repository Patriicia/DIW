...
<?php
     ...
     $sql = "SELECT * FROM usuarios
      WHERE Usuario_email='%$Usuario_email%' OR Usuario_provincia='$Usuario_provincia' OR Usuario_nif='$Usuario_nif'";
...
$resultsNull = $conn->query($sqlNull);
                    ...
                    WHERE Usuario_email='%$Usuario_email%' OR Usuario_provincia='$Usuario_provincia' OR Usuario_nif='$Usuario_nif' LIMIT ". $primeraPagina . ',' . $resultadoPorPaginas;
             ...
             <nav aria-label="Page navigation example">
                            <ul class="pagination">
                                <li class="page-item <?php echo $pagina == 1 ? 'disabled' : '' ?>">
                                    <a class="page-link" href="gestionUsuarioConsulta.php?pagina=<?php echo $pagina - 1; ?>" aria-label="Previous">
                                ...
                                <li class="page-item <?php echo $pagina >= $paginacion ? 'disabled' : '' ?>">
                                    <a class="page-link" href="<?php echo 'gestionUsuarioConsulta.php?pagina=' . $pagina + 1; ?>" aria-label="Next">
...
function seleccionar_todo(){
    for(x=0; x<document.f1.elements.length;x++){
        if(document.f1.elements[x].type=='checkbox'){
            document.f1.elements[x].checked=1;
        }
    }
}
...
function deseleccionar_todo(){
    for(x=0; x<document.f1.elements.length;x++){
        if(document.f1.elements[x].type=='checkbox'){
            document.f1.elements[x].checked=0;
        }
    }
}
